...
class TopLevelBackStack<T : NavKey>(
    ...
    private var topLevelBackStacks: HashMap<T, SnapshotStateList<T>> =
        hashMapOf(
    ...
    fun popToRoot() {
        val root = topLevelBackStacks[topLevelKey]?.firstOrNull()
        if (root != null) {
            topLevelBackStacks[topLevelKey] = mutableStateListOf(root)
            updateBackStack()
        }
    }
}
